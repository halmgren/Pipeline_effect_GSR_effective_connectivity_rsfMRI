function Preprocess_functional_paper_GSR(dataset,slice_time_seconds,session_name,subject,session_one,procedure,SPM_dir,Work_dir)
...
if strcmp(procedure,'Basic') %Same for all procedure, so only done once
    ...
    matlabbatch{1}.spm.temporal.st.ta = (slice_time_seconds-1)*-1*(TR-(TR/image_dim(3))); %ignored if so and reflice in ms
    ...
    %%%%%%%%%%%%%%%%%%%%%%%
    matlabbatch{3}.spm.spatial.coreg.estimate.ref = {[Work_dir '/' dataset '/sub-' sprintf('%03d',subject) '/' session_one '/anat/' procedure '/Skullstr_biascor_structural.nii,1']};
    ...
    matlabbatch{4}.spm.spatial.normalise.write.subj.def = {[Work_dir '/' dataset '/sub-' sprintf('%03d',subject) '/' session_one '/anat/' procedure '/y_structural.nii']};
    ...
    matlabbatch{4}.spm.spatial.normalise.write.woptions.vox = round(abs(imat(7:9)),2);
