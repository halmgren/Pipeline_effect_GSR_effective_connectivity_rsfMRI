function Group_mean_PEB_GSR_regr_A_matrix_paper_GSR(name_ROI_def,ntwrk_name,procedure,N_prec_comp,SPM_dir,Work_dir)
...
for network_number=1:length(ntwrk_name)
    ...
    for number_dataset=1:4
        ...
        for subject=1:number_subject
            ...
            try
                load([Work_dir '/' dataset '/sub-' sprintf('%03d', subject) '_results/DCM/' procedure '/' name_ROI_def '/Full_model/PEB_A_mean_comp_GSR_regr_single_' ntwrk_name{network_number} '.mat']);
            catch ME
                tmp=tmp-1;
                disp('PEB group')
                disp(['Dataset: ' dataset '; Subject: ' num2str(subject) '; Procedure: ' procedure]);
                ...
                if strcmp(ME.identifier,'MATLAB:load:couldNotReadFile')
                    ...
                    save([Work_dir '/Results_paper_GSR/DCM/' procedure '/' name_ROI_def '/Full_model/PEB_group/ERROR_A_mean_' dataset '_subject_' num2str(subject) '_single_' ntwrk_name{network_number} '_comp_GSR_regr.mat'],'ME');
                    continue;
                end
            end
    ...
    save([Work_dir '/Results_paper_GSR/DCM/' procedure '/' name_ROI_def '/Full_model/PEB_group/PEB_A_mean_comp_GSR_regr_group_' cell2mat(N_prec_comp) '_' ntwrk_name{network_number} '.mat'],'PEB','DCM','PEB_group')
    clear PEB_group;
