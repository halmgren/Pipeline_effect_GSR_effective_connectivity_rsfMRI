function Figure_influence_btwn_ntwrk_connectivity_paper_GSR_option4(SPM_dir,Work_dir)
                ...
                for parameter_type={'A'}
                    ...
                    for subnet=1:length(pos_network{network_number})
                        for receiving_subnet=1:length(pos_network{network_number})
                            ...
                            clear PEB;
                            load([Work_dir '/Results_paper_GSR/DCM/Basic/' name_ROI_def '/Full_model/PEB_group/PEB_' parameter_type{:} '_mean_comp_' procedure '_group_' cell2mat(N_prec_comp) '_' ntwrk_abbrev2{network_number} '.mat'],'PEB','DCM','PEB_group');
                            PEB1=PEB;
                            ...
                            load([Work_dir '/Results_paper_GSR/DCM/' procedure '/' name_ROI_def '/Full_model/PEB_group/PEB_' parameter_type{:} '_mean_comp_' procedure '_group_' cell2mat(N_prec_comp) '_' ntwrk_abbrev2{network_number} '.mat'],'PEB','DCM','PEB_group');
                            PEB2=PEB;
...
%                             clear PEB_tmp2;
                            ...
                            clear names;
                            if colored==1
                                saveas(gcf,[direct '/' char(N_prec_comp) '_' parameter_type{:} '_from_' orig_network_abbrev2{network_number}(sum(ntwrk_sizes(1:subnet-1))+1,:) '_to_' orig_network_abbrev2{network_number}(sum(ntwrk_sizes(1:receiving_subnet-1))+1,:) '_colored.bmp']);
                            elseif cntr==1
                                saveas(gcf,[direct '/' char(N_prec_comp) '_' parameter_type{:} '_from_' orig_network_abbrev2{network_number}(sum(ntwrk_sizes(1:subnet-1))+1,:) '_to_' orig_network_abbrev2{network_number}(sum(ntwrk_sizes(1:receiving_subnet-1))+1,:) '_contour_grey.bmp']);
                            elseif colored_2==1
                                saveas(gcf,[direct '/' char(N_prec_comp) '_' parameter_type{:} '_from_' orig_network_abbrev2{network_number}(sum(ntwrk_sizes(1:subnet-1))+1,:) '_to_' orig_network_abbrev2{network_number}(sum(ntwrk_sizes(1:receiving_subnet-1))+1,:) '_colored_2.bmp']);
                            end
                            close;
