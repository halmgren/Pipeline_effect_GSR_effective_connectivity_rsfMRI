function Figure_influence_connectivity_paper_GSR_option4_HCP(SPM_dir,Work_dir)
        ...
        for N_prec_comp={'single'}    %N_prec_comp={'single','fields','all'}
            ...
            for network_number=1:length(ntwrk_name)
                ...
                for parameter_type={'A'}
                    clear PEB;
                    load([Work_dir '/Results_paper_GSR/DCM/Basic/' name_ROI_def '/Full_model/PEB_group/PEB_' parameter_type{:} '_mean_comp_' procedure '_HCP_' cell2mat(N_prec_comp) '_' ntwrk_name{network_number} '.mat'],'PEB','DCM','GCM_full');
                    PEB1=PEB;
                    ...
                    clear PEB PEB_group;
                    load([Work_dir '/Results_paper_GSR/DCM/' procedure '/' name_ROI_def '/Full_model/PEB_group/PEB_' parameter_type{:} '_mean_comp_' procedure '_HCP_' cell2mat(N_prec_comp) '_' ntwrk_name{network_number} '.mat'],'PEB','DCM','GCM_full');
                    PEB2=PEB;
...
%                     Y_axis=Y_axis(:);
                    ...
                    clear names;
                    if colored==1
                        saveas(gcf,[direct '/' char(N_prec_comp) '_' parameter_type{:} '_' ntwrk_name{network_number} '_HCP_colored.bmp']);
                    elseif cntr==1
                        saveas(gcf,[direct '/' char(N_prec_comp) '_' parameter_type{:} '_' ntwrk_name{network_number} '_HCP_contour_grey.bmp']);
                    elseif colored_2==1
                        saveas(gcf,[direct '/' char(N_prec_comp) '_' parameter_type{:} '_' ntwrk_name{network_number} '_HCP_colored_2.bmp']);
                    end
                    close;
