function [hh xb]=plot_ci_adapt12(E,C,x,j,s,barcolor,width,sz1,sz2,GCM,grd,colored,cntr,colored_2,ntwrk_sizes,subnet)
...
%                         for method=1:2
%                             if method==1
                                hh{(regions-1)*sz1+regions2}=bar(ax,[tmp+tmp2:sz1*sz2+sz2],[E(:,tmp) zeros(2,sz1*sz2+sz2-tmp-tmp2)]',width,'Edgecolor',[1 1 1]/2,'Facecolor',barcolor,'BarWidth',0.02);
%                             end
%                         end
                        hold on;
                    end
                ...
                if grd==1
                    for connections=1:length(hh)
                        ...
                        for method=1:2
                            if method==1
                                barclr=rem(connections-1,sz1)+1;
                                hh{connections}(method).FaceColor=[0.2+0.6/sz1*barclr 0.2+0.6/sz1*barclr 0.2+0.6/sz1*barclr];
                        ...
                        for method=1:2
                            ...
                            elseif method==2
                                ...
                                %                         hh{connections}(method).FaceColor=[0 0 1];
                                                        ...
                                                        hh{connections}(method).FaceColor=[0 0+1/(sz1-(sz-4))*(barclr-(sz1-4)) 1];
                                                    end
...
%                 hold(ax,'on');
        ...
        for region=sum(ntwrk_sizes(1:subnet-1))+1:sum(ntwrk_sizes(1:subnet-1))+sz2
            ax.XTick(region-sum(ntwrk_sizes(1:subnet-1)))=[ceil(sz1/2)+(region-sum(ntwrk_sizes(1:subnet-1))-1)*(sz1+1)];
            ax.XTickLabel{region-sum(ntwrk_sizes(1:subnet-1))}=GCM.xY(region).name(5:end);
        end
        ...
        for region=1:sz2-1
            line([(sz1+1)*region (sz1+1)*region],[YL(1) YL(2)],'Color',[0.7 0.7 0.7],'LineStyle','--','LineWidth',2);
        end
%         set(ax,'XLim',[0 n + 1]);
