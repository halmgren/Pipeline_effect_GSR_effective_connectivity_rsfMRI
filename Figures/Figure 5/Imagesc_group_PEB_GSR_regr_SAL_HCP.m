function Imagesc_group_PEB_GSR_regr_SAL_HCP(SPM_dir,Work_dir)

procedure='GSR_regr';
name_ROI_def='Smith';

load([Work_dir '/Results_paper_GSR/DCM/' procedure '/' name_ROI_def '/Full_model/PEB_group/PEB_A_mean_comp_GSR_regr_HCP_single_SAL.mat'],'PEB','DCM','PEB_group');
load([Work_dir '/DatasetHCP/sub-001_summary/DCM/Basic/Smith/Full_model/GCM_SAL_full_estim.mat']);
DCM=GCM;
...
EP=full(vec2mat(PEB.Ep(1:25),5)');
CP=diag(PEB.Cp);
CP=full(vec2mat(CP(1:25),5)');
sgn=sign(EP-ci*sqrt(CP)).*sign(EP+ci*sqrt(CP));

A_matrix=full(vec2mat(PEB.Ep(1:25),5)');
...
if size(A_matrix,1) == size(A_matrix,2) && ...
    ...
    set(gca,'YTickLabel',regions,...
        ...
        'XTickLabel',regions,'fontweight','bold','fontsize',32,'XTick',...
...
A_matrix_NaNs(sgn.*(ones(5,5)-diag([1 1 1 1 1]))==-1)=NaN;


for side1=1:5
    for side2=1:5
        if ~isnan(A_matrix_NaNs(side2,side1))||(side1==side2)
            text(side1,side2,num2str(round(A_matrix(side2,side1),2)),'HorizontalAlignment','center','Fontweight','bold','Fontsize',32);
...
%             text(side1,side2,'','HorizontalAlignment','center','Fontweight','bold','Fontsize',18);
        elseif isnan(A_matrix_NaNs(side2,side1))
            text(side1,side2,num2str(round(A_matrix(side2,side1),2)),'HorizontalAlignment','center','Fontweight','normal','Fontsize',18);
...
for side1=1:5
    for side2=1:5
        ...
        elseif isnan(A_matrix(side1,side2))
...
saveas(gcf,[direct '/Single_A_SAL_matrix_GSR_regr_comp_HCP.bmp']);
close;
