function [hh xb]=plot_ci_adapt11(E,C,x,j,s,barcolor,width,sz,GCM,grd,colored,cntr,colored_2,YL)
        ...
        if strcmpi(s,'exp')
            ...
            if n > 1
                ...
                for regions=1:4
                    ...
                    hh{regions}=bar(ax,[regions+tmp2:4+2]',[E(:,regions) zeros(2,4-regions+1-tmp2+1)]',width,'Edgecolor',[1 1 1]/2,'Facecolor',barcolor,'BarWidth',0.02);
...
%                         end
                ...
                elseif colored_2==1
                    for connections=1:length(hh)
                        for method=1:2
                            barclr=rem(connections-1,length(hh)/2)+1;
                            if method==1
                                if barclr==1
...
%                 hold(ax,'on');
        ...
        %Xl=xlabel('Parameter Type');
        Yl=ylabel('(log) Posterior Expectation');
        ...
        ax.XTick=[];
            ...
            ax.XTick(2)=[4.5];
                ...
                ax.XTickLabel{1}='Endogenous fluctuations';
            
                ax.XTickLabel{2}='(Global) measurement noise';
        ...
        line([3 3],[YL(1) YL(2)],'Color',[0.7 0.7 0.7],'LineStyle','--','LineWidth',2);
